...
HEADER_MANIFEST="MANIFEST.sha256"
...
if [ -z "$VERSION" ]; then
	if ! command -v python3 &>/dev/null; then
		echo "ERROR: python3 is required to resolve the latest mpt-crypto version."
		exit 1
	fi
...
bundle_is_complete() {
	...
	for required in \
		...
		"$INCLUDE_DIR/utility/mpt_utility.h" \
		"$INCLUDE_DIR/$HEADER_MANIFEST"; do
		[ -s "$required" ] || return 1
	done

	(cd "$INCLUDE_DIR" && shasum -a 256 -c "$HEADER_MANIFEST" >/dev/null 2>&1)
}
...
merge_include_tree() {
	...
	while IFS= read -r source_file; do
		...
		destination_file="$STAGE_INCLUDE/$relative_path"
	...
	cp -R "$source_dir/." "$STAGE_INCLUDE/"
	echo "  Staged: $header_count headers from $package_name"
}
...
mkdir -p "$STAGE_LIBS" "$STAGE_INCLUDE"
...
) >"$WORK_DIR/$HEADER_MANIFEST"
mv "$WORK_DIR/$HEADER_MANIFEST" "$STAGE_INCLUDE/$HEADER_MANIFEST"
...
for required in \
	...
	"$STAGE_INCLUDE/utility/mpt_utility.h" \
	"$STAGE_INCLUDE/$HEADER_MANIFEST"; do
	if [ ! -s "$required" ]; then
