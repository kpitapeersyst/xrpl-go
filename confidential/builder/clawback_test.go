...
func TestClawbackBaseValidation(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "fail - X-address holder", base: BuildClawbackParams{Account: testAccount, Holder: xAddressOf(t, testDestination), IssuanceID: testIssuerIssuanceID, IssuerPrivKey: kp.PrivKeyHex}, wantErr: ErrInvalidHolder},
		{name: "fail - X-address account", base: BuildClawbackParams{Account: xAddressOf(t, testAccount), Holder: testDestination, IssuanceID: testIssuerIssuanceID, IssuerPrivKey: kp.PrivKeyHex}, wantErr: ErrInvalidAccount},
